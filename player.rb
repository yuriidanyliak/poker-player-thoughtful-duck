...
class Player
  ...
  def bet_request(game_state)
  	...
  	full_raise =  !minimum_raise.zero? ? minimum_raise +
  	 (game_state["current_buy_in"]..1000).to_a.shuffle.sample : (100..1000).to_a.shuffle.sample
  rescue => err
  	puts err.inspect
 ...
 #  end
  ...
  def test_pair(game_state)
    ...
    all_cards.each {|k,v| pair = true if v==2}
    pair
  end	
